...
const DoctorManagement = () => {
  ...
  const handleAddDoctor = async () => {
    ...
    try {
      ...
      if (formData.middle_name?.trim()) payload.middle_name = formData.middle_name.trim()
      if (formData.emergency_contact?.trim()) payload.emergency_contact = formData.emergency_contact.trim()
      if (formData.shift_timing?.trim()) payload.shift_timing = formData.shift_timing.trim()
      if (formData.joining_date) payload.joining_date = formData.joining_date
      if (formData.address?.trim()) payload.address = formData.address.trim()
      if (formData.department_name?.trim()) payload.department_name = formData.department_name.trim()
      ...
      {isAddMode && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-blue-50 p-4 rounded-lg border border-blue-200">
          ...
          <div>
            ...
            <div className="relative">
              <input className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 pr-10"
                placeholder="Confirm password" type={showConfirmPassword ? 'text' : 'password'} required={isAddMode} value={formData.confirm_password}
