...
import {
  getDoctorInpatientVisits,
  updateDoctorInpatientVitals,
...
const InpatientVisits = () => {
  ...
  const handleSaveVitals = async () => {
    ...
    try {
      ...
      setInpatients((prev) =>
        prev.map((p) => (p.id === selectedPatient.id ? { ...p, vitals } : p)),
      );
      ...
      setInpatients((prev) =>
        prev.map((p) =>
          p.id === selectedPatient.id
            ? {
                ...p,
                medications: [...(p.medications || []), newMedication.trim()],
              }
            : p,
        ),
      );
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            {inpatients.length} admission{inpatients.length !== 1 ? "s" : ""}{" "}
            found
          </p>
        </div>
        ...
        <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg flex items-center gap-3">
          ...
          <div>
            <p className="text-red-700 font-medium">
              Failed to load inpatient visits
            </p>
      ...
      {!error && inpatients.length === 0 && (
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm mt-1">
            {activeOnly
              ? "No active admissions at this time."
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        {inpatients.map((patient) => (
          <div
            key={patient.id}
            className="bg-white p-4 border rounded-xl card-shadow fade-in hover:shadow-md transition-shadow"
          >
            <div className="flex justify-between items-start mb-3">
              ...
              <span
                className={`px-2 py-1 rounded-full text-xs font-medium ${statusColor(patient.status)}`}
              >
                {patient.status}
              </span>
            </div>
            ...
            {patient.vitals && (
              <div className="mb-3 p-3 bg-green-50 rounded-lg border border-green-100 grid grid-cols-2 gap-1 text-xs">
                {patient.vitals.bloodPressure && (
                  <span>
                    <strong>BP:</strong> {patient.vitals.bloodPressure}
                  </span>
                )}
                {patient.vitals.heartRate && (
                  <span>
                    <strong>HR:</strong> {patient.vitals.heartRate}
                  </span>
                )}
                {patient.vitals.temperature && (
                  <span>
                    <strong>Temp:</strong> {patient.vitals.temperature}
                  </span>
                )}
                {patient.vitals.oxygenSaturation && (
                  <span>
                    <strong>SpO₂:</strong> {patient.vitals.oxygenSaturation}
                  </span>
                )}
              </div>
            )}
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">
                Update Notes — {selectedPatient?.name}
              </h3>
              <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <p className="text-xs text-gray-400 mb-4">
              Admission #{selectedPatient?.admissionNumber}
            </p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-lg w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col">
            <div className="flex justify-between items-center p-4 border-b">
              <h3 className="text-lg font-semibold">
                Medications — {selectedPatient?.name}
              </h3>
              <button
                ...
                <div className="space-y-2">
                  {selectedPatient.medications.map((med, index) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          handleRemoveMedication(selectedPatient.id, index)
                        }
