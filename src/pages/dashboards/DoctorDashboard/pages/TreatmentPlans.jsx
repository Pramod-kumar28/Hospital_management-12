...
const PLAN_STATUS_OPTIONS = ['', 'ACTIVE', 'COMPLETED', 'DISCONTINUED', 'DRAFT']
...
const TreatmentPlans = () => {
  ...
  const stats = useMemo(() => {
    return {
      total: plans.length,
      active: plans.filter((plan) => String(plan.status || '').toUpperCase() === 'ACTIVE').length,
      completed: plans.filter((plan) => String(plan.status || '').toUpperCase() === 'COMPLETED').length,
      avgProgress:
        plans.length > 0
          ? Math.round(
              plans.reduce((sum, plan) => sum + Number(plan.progress_percentage || 0), 0) / plans.length
            )
          : 0,
    }
  }, [plans])
  ...
  const handleOpenPlan = async (plan) => {
    ...
    try {
      ...
      if (outcomeRes.ok && !outcomePayload?.message?.startsWith('No treatment outcome assessment')) {
  ...
  const handleCreatePlan = async () => {
    try {
      const body = {
        ...
        short_term_goals: parseJsonArray(createForm.short_term_goals, 'Short term goals'),
        long_term_goals: parseJsonArray(createForm.long_term_goals, 'Long term goals'),
        interventions: parseJsonArray(createForm.interventions, 'Interventions'),
        milestones: parseJsonArray(createForm.milestones, 'Milestones'),
      }

      if (!body.patient_ref || !body.plan_name || !body.primary_diagnosis) {
        toast.info('Patient ref, plan name and primary diagnosis are required.')
        return
      }
  ...
  const handleMarkCompleted = async (plan) => {
    ...
    try {
      ...
      await loadPlans()
      if (selectedPlan && (selectedPlan.plan_id || selectedPlan.id) === planId) {
  ...
  const handleSaveProgress = async () => {
    ...
    try {
      ...
      setProgressForm({ progress_note: '', significant_change: false, next_review_date: '' })
      await loadPlans()
  ...
  const handleSaveOutcome = async () => {
    ...
    try {
      const response = await recordDoctorTreatmentPlanOutcome(planId, {
        ...
        goals_partially_achieved: Number(outcomeForm.goals_partially_achieved) || 0,
        goals_not_achieved: Number(outcomeForm.goals_not_achieved) || 0,
        treatment_adherence: Number(outcomeForm.treatment_adherence) || 0,
        patient_satisfaction_score: Number(outcomeForm.patient_satisfaction_score) || 0,
        quality_of_life_score: Number(outcomeForm.quality_of_life_score) || 0,
        complications: parseJsonArray(outcomeForm.complications, 'Complications'),
        future_recommendations: parseJsonArray(outcomeForm.future_recommendations, 'Future recommendations'),
      })
      const payload = await response.json().catch(() => ({}))
      if (!response.ok) {
        toast.error(doctorAppointmentErrorMessage(payload))
  ...
  const handleCreateFromTemplate = async () => {
    ...
    try {
      ...
      toast.success(payload?.message || 'Treatment plan created from template.')
      await loadPlans()
    } catch {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-gray-800">Treatment Plan Management</h2>
          <p className="text-sm text-gray-500">
            Create, monitor, update outcomes, templates and analytics for treatment plans
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button className="btn-secondary" onClick={handleCleanupDates} disabled={submitLoading}>
      ...
      <div className="bg-white rounded-xl border p-5 space-y-4">
        <div className="flex flex-col lg:flex-row lg:items-end gap-3">
          ...
          <div className="flex-1">
            <label className="block text-sm text-gray-700 mb-1">Patient Ref</label>
            <input
              className="form-input"
              value={filters.patient_ref}
              onChange={(e) => setFilters((prev) => ({ ...prev, patient_ref: e.target.value }))}
              placeholder="PT-0001"
            />
          </div>
          <div>
          ...
          <button className="btn-primary" onClick={loadPlans}>
            Apply Filters
          </button>
        </div>
        <DataTable
          columns={[
            ...
            },
            {
              ...
              title: 'Status',
              render: (value) => (
                <span className="px-2 py-1 rounded-full text-xs bg-gray-100 text-gray-700">{value || '-'}</span>
              ),
            },
            {
              key: 'created_date',
              ...
              render: (_, row) => (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    disabled={String(row?.status || '').toUpperCase() === 'COMPLETED' || submitLoading}
                    onClick={(e) => {
                      e.stopPropagation()
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border p-5 space-y-4">
          <h3 className="text-lg font-semibold text-gray-800">Plan Templates</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <input
              ...
              onChange={(e) => setTemplateForm((prev) => ({ ...prev, specialty: e.target.value }))}
            />
            <input
              className="form-input"
              placeholder="Template ID"
              value={templateForm.template_id}
              onChange={(e) => setTemplateForm((prev) => ({ ...prev, template_id: e.target.value }))}
            />
            <input
              className="form-input"
              placeholder="Patient Ref for template create"
              value={templateForm.patient_ref}
              onChange={(e) => setTemplateForm((prev) => ({ ...prev, patient_ref: e.target.value }))}
            />
          </div>
          <div className="flex gap-2">
            <button className="btn-secondary" onClick={handleLoadTemplates}>
              {templatesLoading ? 'Loading...' : 'Load Templates'}
            </button>
            <button className="btn-primary" onClick={handleCreateFromTemplate} disabled={submitLoading}>
        ...
        <div className="bg-white rounded-xl border p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input
              ...
              onChange={(e) => setAnalyticsFilters((prev) => ({ ...prev, date_from: e.target.value }))}
            />
            <input
              className="form-input"
              type="date"
              value={analyticsFilters.date_to}
              onChange={(e) => setAnalyticsFilters((prev) => ({ ...prev, date_to: e.target.value }))}
            />
          </div>
          <button className="btn-primary" onClick={handleLoadAnalytics}>
            {analyticsLoading ? 'Loading...' : 'Load Analytics'}
          </button>
          <div className="bg-gray-50 rounded-lg p-4 text-sm space-y-1">
            <p><strong>Doctor:</strong> {analytics?.doctor_name || '-'}</p>
            <p><strong>Period:</strong> {analytics?.analysis_period || '-'}</p>
            <p><strong>Total Plans:</strong> {analytics?.summary_statistics?.total_plans ?? '-'}</p>
            <p><strong>Success Rate:</strong> {analytics?.summary_statistics?.success_rate ?? '-'}%</p>
            <p>
              <strong>Avg Completion:</strong> {analytics?.summary_statistics?.average_completion_time_days ?? '-'} days
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div className="bg-gray-50 rounded-lg p-4">
                <p><strong>Plan ID:</strong> {planDetails?.plan_id || selectedPlan?.plan_id || '-'}</p>
                <p><strong>Status:</strong> {planDetails?.status || selectedPlan?.status || '-'}</p>
                <p><strong>Patient:</strong> {planDetails?.patient_name || selectedPlan?.patient_name || '-'}</p>
                <p><strong>Patient Ref:</strong> {planDetails?.patient_ref || selectedPlan?.patient_ref || '-'}</p>
                <p><strong>Primary Diagnosis:</strong> {planDetails?.primary_diagnosis || selectedPlan?.primary_diagnosis || '-'}</p>
                <p><strong>Progress:</strong> {planDetails?.progress_percentage ?? selectedPlan?.progress_percentage ?? 0}%</p>
              </div>
              <div className="bg-gray-50 rounded-lg p-4">
                <p><strong>Total Goals:</strong> {planStatus?.total_goals ?? '-'}</p>
                <p><strong>Total Milestones:</strong> {planStatus?.total_milestones ?? '-'}</p>
                <p><strong>Progress Notes:</strong> {planStatus?.total_progress_notes ?? '-'}</p>
                <p><strong>Has Outcome:</strong> {planStatus?.has_outcome_assessment ? 'Yes' : 'No'}</p>
                <p><strong>Outcome Date:</strong> {planStatus?.outcome_assessment_date || '-'}</p>
              </div>
            </div>

            <div>
              <h4 className="text-base font-semibold text-gray-800 mb-2">Goals & Milestones</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <div className="border rounded-lg p-3">
                  <p className="font-medium mb-2">Short-Term Goals</p>
                  <ul className="list-disc pl-5">
                    {(planDetails?.short_term_goals || []).slice(0, 5).map((goal) => (
                      <li key={goal.goal_id || goal.description}>{goal.description || '-'}</li>
                    ))}
                  </ul>
                </div>
                <div className="border rounded-lg p-3">
            ...
            <div className="bg-gray-50 rounded-lg p-4 text-sm">
              <h4 className="text-base font-semibold text-gray-800 mb-2">Outcome Summary</h4>
              {planOutcome ? (
                <div className="space-y-1">
                  <p><strong>Overall Outcome:</strong> {planOutcome?.overall_outcome || '-'}</p>
                  <p><strong>Clinical Effectiveness:</strong> {planOutcome?.clinical_effectiveness || '-'}</p>
                  <p><strong>Patient Satisfaction:</strong> {planOutcome?.patient_satisfaction_score ?? '-'}</p>
                  <p><strong>Summary:</strong> {planOutcome?.outcome_summary || '-'}</p>
                </div>
              ) : (
                <p className="text-gray-500">No outcome assessment recorded yet.</p>
              )}
            </div>

            <div className="flex flex-wrap justify-end gap-2">
              <button className="btn-secondary" onClick={() => setShowProgressModal(true)}>
                Update Progress
              </button>
              <button className="btn-secondary" onClick={() => setShowOutcomeModal(true)}>
                Record Outcome
              </button>
              <button className="btn-primary" onClick={() => handleMarkCompleted(selectedPlan)} disabled={submitLoading}>
                Mark Completed
              </button>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input
              ...
              onChange={(e) => setCreateForm((prev) => ({ ...prev, patient_ref: e.target.value }))}
            />
            <input
              className="form-input"
            ...
            <input
              ...
              onChange={(e) => setCreateForm((prev) => ({ ...prev, primary_diagnosis: e.target.value }))}
            />
          </div>
          <textarea
            className="form-input"
          ...
          <textarea
            ...
            onChange={(e) => setCreateForm((prev) => ({ ...prev, short_term_goals: e.target.value }))}
          />
          <textarea
            className="form-input"
            rows={3}
            ...
            onChange={(e) => setCreateForm((prev) => ({ ...prev, long_term_goals: e.target.value }))}
          />
          <textarea
            className="form-input"
            rows={3}
            ...
            onChange={(e) => setCreateForm((prev) => ({ ...prev, interventions: e.target.value }))}
          />
          <textarea
            className="form-input"
            rows={3}
          ...
          <div className="flex justify-end gap-2">
            ...
            <button className="btn-primary" onClick={handleCreatePlan} disabled={submitLoading}>
      ...
      >
        <div className="space-y-4">
          <textarea
            ...
            onChange={(e) => setProgressForm((prev) => ({ ...prev, progress_note: e.target.value }))}
          />
          <input
            className="form-input"
            type="date"
            value={progressForm.next_review_date}
            onChange={(e) => setProgressForm((prev) => ({ ...prev, next_review_date: e.target.value }))}
          />
          <label className="inline-flex items-center gap-2 text-sm text-gray-700">
            <input
              type="checkbox"
              checked={progressForm.significant_change}
              onChange={(e) => setProgressForm((prev) => ({ ...prev, significant_change: e.target.checked }))}
            />
            Significant change
          </label>
          <div className="flex justify-end gap-2">
            <button className="btn-secondary" onClick={() => setShowProgressModal(false)}>Cancel</button>
            <button className="btn-primary" onClick={handleSaveProgress} disabled={submitLoading}>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input
              ...
              onChange={(e) => setOutcomeForm((prev) => ({ ...prev, outcome_date: e.target.value }))}
            />
            <select
              className="form-input"
              value={outcomeForm.overall_outcome}
              onChange={(e) => setOutcomeForm((prev) => ({ ...prev, overall_outcome: e.target.value }))}
            >
            ...
            <input
              ...
              onChange={(e) => setOutcomeForm((prev) => ({ ...prev, goals_achieved: e.target.value }))}
            />
            <input
              className="form-input"
              type="number"
              ...
              onChange={(e) => setOutcomeForm((prev) => ({ ...prev, goals_partially_achieved: e.target.value }))}
            />
            <input
              className="form-input"
              type="number"
              ...
              onChange={(e) => setOutcomeForm((prev) => ({ ...prev, goals_not_achieved: e.target.value }))}
            />
            <input
              className="form-input"
              type="number"
              ...
              onChange={(e) => setOutcomeForm((prev) => ({ ...prev, treatment_adherence: e.target.value }))}
            />
          </div>
          <textarea
            className="form-input"
            ...
            onChange={(e) => setOutcomeForm((prev) => ({ ...prev, outcome_summary: e.target.value }))}
          />
          <textarea
            className="form-input"
            rows={2}
            ...
            onChange={(e) => setOutcomeForm((prev) => ({ ...prev, complications: e.target.value }))}
          />
          <textarea
            className="form-input"
            rows={2}
            ...
            onChange={(e) => setOutcomeForm((prev) => ({ ...prev, future_recommendations: e.target.value }))}
          />
          <input
            className="form-input"
            placeholder="Follow-up plan"
            value={outcomeForm.follow_up_plan}
            onChange={(e) => setOutcomeForm((prev) => ({ ...prev, follow_up_plan: e.target.value }))}
          />
          <textarea
            className="form-input"
            rows={2}
          ...
          />
          <div className="flex justify-end gap-2">
            <button className="btn-secondary" onClick={() => setShowOutcomeModal(false)}>Cancel</button>
            <button className="btn-primary" onClick={handleSaveOutcome} disabled={submitLoading}>
