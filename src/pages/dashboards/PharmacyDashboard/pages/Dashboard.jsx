...
const handleNewOrder = () =>
  toast.success("Redirecting to new purchase order");

const handleFilterChange = (e) =>
  toast(`Filter applied: ${e.target.value}`);

  return (
<div className="bg-slate-100 min-h-screen w-full text-left space-y-6 px-3 sm:px-6 py-4">
    ...
    {/* TOP STATS (Modern Analytics Style) */}
  ...
  />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[500px]">
              ...
              <tbody className="divide-y">
                <StockRow name="Amoxicillin 500mg" category="Antibiotic" stock="12 units" />
                <StockRow name="Paracetamol 650mg" category="Analgesic" stock="8 units" />
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow p-4 sm:p-6">
          ...
          <button onClick={handleNewOrder} className="mt-6 w-full bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700">
            + New Order
          </button>
        </div>
      </div>
...
const StatCard = ({ title, value, note, icon: Icon, iconBg, actionText, onAction }) => (
  <div className="bg-white rounded-xl shadow p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
      ...
      {actionText && (
        <button onClick={onAction} className="text-xs text-blue-600 mt-2">
          {actionText} →
        </button>
      )}
    </div>
...
const StockRow = ({ name, category, stock }) => (
