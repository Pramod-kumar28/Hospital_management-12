...
//       setDoctors([
//         { id: 'D-001', name: 'Dr. Meena Rao', department: 'Cardiology', opdRoom: 'Room 101', currentPatient: 'T-102', queue: 3 },
//         { id: 'D-002', name: 'Dr. Sharma', department: 'Orthopedics', opdRoom: 'Room 102', currentPatient: 'T-103', queue: 2 },
//         { id: 'D-003', name: 'Dr. Menon', department: 'Neurology', opdRoom: 'Room 103', currentPatient: null, queue: 1 }
//       ]);
...
//       case 'In Consultation': return 'status-processing';
...
//     return priority === 'Urgent' ? 'status-urgent' : 'bg-gray-100 text-gray-800';
//   };
...
//         <h2 className="text-xl md:text-2xl font-semibold text-gray-700">🩺 OPD Management</h2>
...
//             </div>
//             <div>
//               <p className="text-gray-500">Today's OPD</p>
//               <p className="text-2xl font-bold text-blue-600 mt-1">{opdPatients.length}</p>
//             </div>
//           </div>
//         </div>
...
//               <p className="text-2xl font-bold text-green-600 mt-1">{doctors.length}</p>
//             </div>
//           </div>
//         </div>
...
//             </div>
//             <div>
//               <p className="text-gray-500">Avg Waiting Time</p>
//               <p className="text-2xl font-bold text-yellow-600 mt-1">18 mins</p>
//             </div>
//           </div>
//         </div>
...
//         <DataTable
//           columns={[
//             { key: 'token', title: 'Token No', sortable: true },
//             { 
//               key: 'patientName', 
//               title: 'Patient', 
//               sortable: true,
...
//             { key: 'waitingTime', title: 'Waiting', sortable: true },
//             { 
//               key: 'priority', 
//               title: 'Priority', 
//               sortable: true,
...
//                     title="Transfer"
//                     onClick={() => alert(`Transfer ${row.patientName}`)}
//                   >
...
//                     title="Cancel"
//                     onClick={() => {
//                       if (window.confirm(`Cancel token ${row.token}?`)) {
//                         setOpdPatients(opdPatients.filter(p => p.id !== row.id));
//                       }
//                     }}
//                   >
...
//         <h3 className="text-lg font-semibold mb-4">👨‍⚕️ Doctors Status</h3>
...
//                   <span className="font-medium">{doctor.currentPatient || 'None'}</span>
//                 </div>
//                 <div className="flex justify-between">
//                   <span className="text-gray-600">Queue:</span>
//                   <span className="font-medium">{doctor.queue} patients</span>
//                 </div>
//               </div>
//               <div className="mt-3 pt-3 border-t">
//                 <button className="w-full btn btn-outline btn-sm">
//                   <i className="fas fa-eye mr-1"></i> View Queue
//                 </button>
//               </div>
//             </div>
//           ))}
...
//                   <p className="font-bold">{selectedPatient.token}</p>
//                 </div>
//                 <div>
//                   <p className="text-sm text-gray-600">Doctor</p>
//                   <p className="font-bold">{selectedPatient.doctor}</p>
//                 </div>
//                 <div>
//                   <p className="text-sm text-gray-600">Department</p>
//                   <p className="font-bold">{selectedPatient.department}</p>
//                 </div>
//               </div>
//             </div>
...
//                 </span>
//               </div>
//               <p className="text-gray-600 mb-3">{doctor.department} - {doctor.opdRoom}</p>
              
//               <div className="space-y-2">
//                 {opdPatients
//                   .filter(p => p.doctor === doctor.name && p.status === 'Waiting')
//                   .map(patient => (
//                     <div key={patient.id} className="flex items-center justify-between bg-gray-50 p-2 rounded">
//                       <div>
//                         <span className="font-medium">{patient.token}</span>
//                         <span className="ml-3">{patient.patientName}</span>
//                       </div>
//                       <div className="text-sm text-gray-600">
//                         Wait: {patient.waitingTime}
//                       </div>
//                     </div>
//                   ))}
//               </div>
              
//               {opdPatients.filter(p => p.doctor === doctor.name && p.status === 'Waiting').length === 0 && (
//                 <p className="text-gray-500 text-center py-2">No patients in queue</p>
//               )}
//             </div>
//           ))}
//         </div>
        ...
        <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-800 mt-1">{opdPatients.length}</p>
            </div>
          </div>
        ...
        <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-800 mt-1">{activeDoctors.length}</p>
            </div>
          </div>
          <div className="mt-3 pt-3 border-t border-gray-100">
            <span className="text-sm text-gray-600">
        ...
        <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold text-gray-800 mt-1">{calculateAverageWaitingTime()}</p>
            </div>
          </div>
          <div className="mt-3 pt-3 border-t border-gray-100">
            <span className="text-sm text-gray-600">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <input
                    ...
                    className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 text-sm"
                  />
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="py-3 px-4 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Queue</th>
                  ...
                  <tr key={patient.id} className="hover:bg-gray-50">
                    <td className="py-4 px-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-xs text-gray-500">{patient.token}</span>
                    ...
                    <td className="py-4 px-4">
                      {patient.status === 'Waiting' && patient.assignedDoctorId ? (
                        <div className="flex items-center">
                          <span className="w-7 h-7 bg-yellow-100 text-yellow-800 rounded-full flex items-center justify-center text-sm font-medium mr-2">
                            {patient.queuePosition + 1}
                          </span>
                          <span className="text-sm text-gray-700">{patient.waitingTime}</span>
                        </div>
                      ) : patient.status === 'In Consultation' ? (
                        <span className="text-sm text-blue-600 font-medium">Consulting Now</span>
                      ) : (
                    ...
                    <td className="py-4 px-4">
                      <div className="space-y-1">
                        ...
                        {patient.priority === 'Urgent' && (
                          <span className="block px-2 py-1 bg-red-50 text-red-700 rounded text-xs font-medium w-fit">
                            Urgent
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        {patient.status === 'In Consultation' && (
                          <button 
                            className="text-blue-600 hover:text-blue-800 p-1"
                            onClick={() => handleStartConsultation(patient)}
                            title="View/Continue Consultation"
                          >
                            <i className="fas fa-eye"></i>
                          </button>
                        )}
